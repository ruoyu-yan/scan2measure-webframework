...
def main():
    ...
    # --- Global Linear Assignment (Hungarian) with scale hypothesis testing ---
    # 1) Generate candidate scales from all RF/LGT diagonal ratios
    rf_diags = np.array([bounding_box_diagonal(p) for p in rf_polys], dtype=float)
    lgt_diags = np.array([bounding_box_diagonal(r["poly"]) for r in lgt_rooms], dtype=float)

    valid_rf = rf_diags > 1e-12
    valid_lgt = lgt_diags > 1e-12
    if not np.any(valid_rf) or not np.any(valid_lgt):
        print("\n[Error] Invalid diagonals; cannot generate scale candidates.")
        return

    ratio_matrix = (rf_diags[None, :] / lgt_diags[:, None])
    ratio_matrix = ratio_matrix[np.ix_(valid_lgt, valid_rf)]
    ...
    # Deduplicate to keep runtime reasonable while still testing the hypothesis set
    candidate_scales = np.unique(np.round(np.array(candidate_scales, dtype=float), 4))

    tol = 0.10  # strict tolerance (10%)
    BIG_COST = 1e9
    ...
    print(f"\nTesting {len(candidate_scales)} candidate scales...")

    # Precompute full ratio matrix for gating (all pairs)
    full_ratio = rf_diags[None, :] / lgt_diags[:, None]
    ...
    for s in candidate_scales:
        ...
        # Only compute expensive Chamfer costs for gated pairs
        for i_room, room in enumerate(lgt_rooms):
            ...
            for j_rf, rf_poly in enumerate(rf_polys):
                if not gate[i_room, j_rf]:
                    continue
                try:
                    _, _, err = align_polygons(rf_poly, lgt_poly, force_scale=s)
                except Exception:
                    continue
                cost[i_room, j_rf] = err

        row_ind, col_ind = linear_sum_assignment(cost)
        assigned_costs = cost[row_ind, col_ind]
        valid_mask = assigned_costs < BIG_COST / 2
        valid_count = int(np.sum(valid_mask))
        total_error = float(np.sum(assigned_costs[valid_mask]))

        if (valid_count > best_valid_count) or (
            valid_count == best_valid_count and total_error < best_total_error
        ):
            best_valid_count = valid_count
            best_total_error = total_error
            best_scale = float(s)
            best_assignment = (row_ind, col_ind, valid_mask)
    ...
    print(
        f"\nBest Scale: {best_scale:.4f} | Valid Matches: {best_valid_count} | Total Error: {best_total_error:.4f}"
    )
    ...
    for i_room, room in enumerate(lgt_rooms):
        ...
        for j_rf, rf_poly in enumerate(rf_polys):
            if not final_gate[i_room, j_rf]:
                continue
            try:
                _, _, err = align_polygons(rf_poly, lgt_poly, force_scale=best_scale)
            except Exception:
                continue
            final_cost[i_room, j_rf] = err
    ...
    for i_room, j_rf in zip(row_ind, col_ind):
        ...
        aligned, params, err = align_polygons(rf_poly, room["poly"], force_scale=best_scale)
    ...
    ax_map.set_title("Align Result (Hungarian Assignment)")
